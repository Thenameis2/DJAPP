...
type DeckId = "a" | "b";
type Track = { id: number; path: string; title: string; artist: string | null; durationSeconds: number | null; codec: string | null; missing: boolean; analysisStatus: string | null; analysisError: string | null; bpm: number | null; bpmConfidence: number | null; bpmCorrected: boolean; musicalKey: string | null; keyConfidence: number | null; keyCorrected: boolean; waveformAvailable: boolean; beatGridAvailable: boolean };
type AnalysisJob = { trackId: number; stage: string; completedFraction: number | null; message: string | null; queuePosition: number | null };
type AnalyzeAllResult = { queued: number; alreadyQueued: number; current: number; missing: number; unreadable: number; queueFull: boolean };
...
const activeAnalysisStages = new Set(["queued", "decoding", "waveform", "rhythm", "key", "loudness", "writing"]);
...
const keyLabel = (value: string | null) => {
  if (!value) return "--";
  const [pitch, mode] = value.split(":");
  const note = ["C", "C♯", "D", "E♭", "E", "F", "F♯", "G", "A♭", "A", "B♭", "B"][Number(pitch)];
  return note ? `${note}${mode === "minor" ? "m" : ""}` : value;
};

const parseKeyCorrection = (value: string) => {
  const normalized = value.trim().replace("♯", "#").replace("♭", "b");
  if (!normalized) return null;
  const match = normalized.match(/^([A-Ga-g])([#b]?)(m|minor|major)?$/);
  if (!match) throw new Error("Use a key such as C, F# or Am.");
  ...
  return `${pitchClasses[note]}:${match[3] === "m" || match[3] === "minor" ? "minor" : "major"}`;
};
...
function App() {
  ...
  const [analysisStatus, setAnalysisStatus] = useState("Analysis is ready.");
  const [analysisQueueOpen, setAnalysisQueueOpen] = useState(false);
  const [analysisQueueDismissed, setAnalysisQueueDismissed] = useState(false);
  ...
  useEffect(() => { let cancelled = false; const refresh = async () => { try { const jobs = await invoke<AnalysisJob[]>("analysis_jobs"); if (!cancelled) { const hasActiveJobs = jobs.some((job) => activeAnalysisStages.has(job.stage)); setAnalysisJobs(jobs); if (hasActiveJobs) { if (!analysisQueueDismissed) setAnalysisQueueOpen(true); await refreshLibrary(); } else { setAnalysisQueueDismissed(false); } } } catch (error: unknown) { if (!cancelled) setAnalysisStatus(`Analysis unavailable: ${String(error)}`); } }; void refresh(); const timer = window.setInterval(() => void refresh(), 1000); return () => { cancelled = true; window.clearInterval(timer); }; }, [analysisQueueDismissed]);
  ...
  const openAnalysisQueue = () => { setAnalysisQueueDismissed(false); setAnalysisQueueOpen(true); };
  const closeAnalysisQueue = () => { setAnalysisQueueDismissed(true); setAnalysisQueueOpen(false); };
  const analyzeTrack = async (track: Track) => { openAnalysisQueue(); try { await invoke("analysis_analyze_track", { trackId: track.id }); setAnalysisStatus(`Queued analysis for ${track.title}.`); await refreshLibrary(); } catch (error: unknown) { setAnalysisStatus(`Could not analyze ${track.title}: ${String(error)}`); } };
  const cancelAnalysisById = async (trackId: number, title = "track") => { try { await invoke("analysis_cancel_track", { trackId }); setAnalysisStatus(`Cancelled analysis for ${title}.`); await refreshLibrary(); } catch (error: unknown) { setAnalysisStatus(`Could not cancel analysis: ${String(error)}`); } };
  const cancelAnalysis = async (track: Track) => cancelAnalysisById(track.id, track.title);
  const analyzeAll = async () => { openAnalysisQueue(); try { const result = await invoke<AnalyzeAllResult>("analysis_analyze_all"); const parts = [`Queued ${result.queued}`, `${result.alreadyQueued} already queued/current`, `${result.current} current`, `${result.missing} missing`, `${result.unreadable} unreadable`]; setAnalysisStatus(`${parts.join(" · ")}.${result.queueFull ? " Queue is full; run Analyze missing again as jobs finish." : ""}`); await refreshLibrary(); } catch (error: unknown) { setAnalysisStatus(`Could not queue library analysis: ${String(error)}`); } };
  const correctAnalysis = async (track: Track) => { try { const bpmText = window.prompt("Correct BPM. Leave blank to use generated BPM.", track.bpmCorrected && track.bpm !== null ? String(track.bpm) : ""); if (bpmText === null) return; const keyText = window.prompt("Correct key, for example C, F# or Am. Leave blank to use generated key.", track.keyCorrected ? keyLabel(track.musicalKey) : ""); if (keyText === null) return; const bpm = bpmText.trim() ? Number(bpmText) : null; if (bpm !== null && (!Number.isFinite(bpm) || bpm <= 0)) throw new Error("BPM must be positive."); await invoke("analysis_set_correction", { trackId: track.id, bpm, musicalKey: parseKeyCorrection(keyText), beatGridOffsetFrames: null }); setAnalysisStatus(`Updated corrections for ${track.title}.`); await refreshLibrary(); } catch (error: unknown) { setAnalysisStatus(`Could not update correction: ${String(error)}`); } };
  const activeAnalysis = (trackId: number) => analysisJobs.find((job) => job.trackId === trackId && activeAnalysisStages.has(job.stage));
  const tracksById = new Map(tracks.map((track) => [track.id, track]));
  const orderedAnalysisJobs = [...analysisJobs].sort((a, b) => (a.queuePosition ?? Number.MAX_SAFE_INTEGER) - (b.queuePosition ?? Number.MAX_SAFE_INTEGER) || a.trackId - b.trackId);
  const activeQueueCount = analysisJobs.filter((job) => activeAnalysisStages.has(job.stage)).length;
  ...
  return <main className="app-shell">
    ...
    <section className="browser"><aside className="library-sidebar"><strong>MY FILES</strong><button className="add-folder" onClick={addMusicFolder} disabled={isScanning}>＋ Add folder</button><span>Local Music</span><span>Recently Added</span><span>Missing Files</span></aside><div className="library-main"><header><div><strong>Music Library</strong><small>{libraryStatus} · {analysisStatus}</small></div><div className="library-header-actions"><button onClick={openAnalysisQueue} disabled={analysisJobs.length === 0}>View queue{activeQueueCount > 0 ? ` (${activeQueueCount})` : ""}</button><button onClick={analyzeAll}>Analyze missing</button><button onClick={addMusicFolder} disabled={isScanning}>{isScanning ? "Scanning..." : "Scan folder"}</button></div></header><div className="track-table-wrap"><table className="track-table"><thead><tr><th>#</th><th>Title</th><th>Artist</th><th>BPM</th><th>Key</th><th>Analysis</th><th>Time</th><th>Load</th></tr></thead><tbody>{tracks.length === 0 ? <tr><td colSpan={8} className="empty-library">Add a folder containing MP3, WAV, FLAC, AAC/M4A, or AIFF files.</td></tr> : tracks.map((track, index) => { const job = activeAnalysis(track.id); const status = job?.stage ?? track.analysisStatus ?? "not analyzed"; return <tr key={track.id} className={track.missing ? "missing-row" : ""}><td>{index + 1}</td><td><strong>{track.title}</strong><small className="track-format">{track.codec?.toUpperCase() ?? "--"}</small></td><td>{track.artist ?? "Unknown artist"}</td><td><span className={track.bpmCorrected ? "corrected-value" : ""}>{track.bpm?.toFixed(1) ?? "--"}</span>{track.bpmConfidence !== null && track.bpmConfidence < 0.65 ? <small className="uncertain">?</small> : null}</td><td><span className={track.keyCorrected ? "corrected-value" : ""}>{keyLabel(track.musicalKey)}</span>{track.keyConfidence !== null && track.keyConfidence < 0.6 ? <small className="uncertain">?</small> : null}</td><td><span className={`analysis-state analysis-${status}`}>{status}</span>{track.analysisError ? <small className="analysis-error" title={track.analysisError}>!</small> : null}<div className="analysis-actions">{job ? <button onClick={() => cancelAnalysis(track)}>Cancel</button> : <button onClick={() => analyzeTrack(track)} disabled={track.missing}>Analyze</button>}<button onClick={() => correctAnalysis(track)}>Correct</button></div></td><td>{formatDuration(track.durationSeconds)}</td><td><div className="table-actions"><button onClick={() => runDeckCommand("a", "deck_a_load", { trackId: track.id }, `Loaded ${track.title} into Deck A.`)} disabled={track.missing || busyDeck !== null}>A</button><button onClick={() => runDeckCommand("b", "deck_b_load", { trackId: track.id }, `Loaded ${track.title} into Deck B.`)} disabled={track.missing || busyDeck !== null}>B</button></div></td></tr>; })}</tbody></table></div></div><aside className="automix-panel"><strong>AUTOMIX</strong><button disabled>Start AutoMix</button><p>Queue and intelligent transitions are planned for the AutoMix milestone.</p><small>Audio health<br />Callbacks {mixer.callbacks}<br />Clipped {mixer.clippedSamples}<br />Errors {mixer.streamErrors}<br />Cue queue {mixer.cueQueueDepthFrames}<br />Cue underflows {mixer.cueUnderflowCallbacks}<br />Cue errors {mixer.cueStreamErrors}</small></aside></section>
    {analysisQueueOpen ? <div className="analysis-modal-backdrop" role="presentation"><section className="analysis-modal" role="dialog" aria-modal="true" aria-label="Analyze queue"><header><div><strong>Analyze Queue</strong><small>{activeQueueCount > 0 ? `${activeQueueCount} track${activeQueueCount === 1 ? "" : "s"} active or waiting` : "No active analysis jobs"}</small></div><button type="button" onClick={closeAnalysisQueue}>Close</button></header><p className="analysis-modal-note">Analysis runs one track at a time so decoding, waveform, BPM, key, and cache writes do not compete with playback.</p><ol className="analysis-job-list">{orderedAnalysisJobs.length === 0 ? <li className="analysis-job-empty">No analysis jobs yet. Use Analyze or Analyze missing to start the queue.</li> : orderedAnalysisJobs.map((job) => { const track = tracksById.get(job.trackId); const progress = job.completedFraction === null ? null : Math.round(job.completedFraction * 100); const isActive = activeAnalysisStages.has(job.stage); return <li className={`analysis-job-row ${isActive ? "analysis-job-active" : ""}`} key={`${job.trackId}-${job.stage}`}><span className="queue-position">{job.queuePosition === 0 ? "Now" : job.queuePosition === null ? "Done" : `#${job.queuePosition}`}</span><div className="queue-track"><strong>{track?.title ?? `Track ${job.trackId}`}</strong><small>{track?.artist ?? "Unknown artist"} · BPM {track?.bpm?.toFixed(1) ?? "--"}{track?.bpmConfidence !== null && track?.bpmConfidence !== undefined ? ` · confidence ${Math.round(track.bpmConfidence * 100)}%` : ""}</small>{job.message ? <small>{job.message}</small> : null}</div><span className={`analysis-state analysis-${job.stage}`}>{job.stage}</span><span className="queue-progress">{progress === null ? "--" : `${progress}%`}</span>{isActive ? <button type="button" onClick={() => cancelAnalysisById(job.trackId, track?.title)}>Cancel</button> : <span />}</li>; })}</ol></section></div> : null}
  </main>;
}
