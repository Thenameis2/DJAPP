...
type DeckSnapshot = { loadedTrackId: number | null; title: string | null; durationSeconds: number | null; positionSeconds: number; state: "paused" | "playing" | "ended"; underflowCallbacks: number; streamErrors: number; workerError: string | null };
...
const unloadedDeck: DeckSnapshot = { loadedTrackId: null, title: null, durationSeconds: null, positionSeconds: 0, state: "paused", underflowCallbacks: 0, streamErrors: 0, workerError: null };
...
type DeckPanelProps = { id: DeckId; snapshot: DeckSnapshot; seek: number; gain: number; cued: boolean; busy: boolean; message: string; onSeekChange: (value: number) => void; onCommand: (command: string, args?: Record<string, unknown>, success?: string) => void; onGain: (gain: number) => void };

function DeckPanel({ id, snapshot, seek, gain, cued, busy, message, onSeekChange, onCommand, onGain }: DeckPanelProps) {
  ...
  return <article className={`deck deck-${id}`}>
    ...
    <div className="deck-body">
      <aside className="tempo-rail"><button type="button">SYNC</button><strong>100.0</strong><span>0.0%</span><input type="range" min={0} max={1} step={0.01} value={gain} onChange={(event) => onGain(Number(event.target.value))} aria-label={`Deck ${label} gain`} /></aside>
    ...
    <small className="deck-feedback">{snapshot.workerError ?? message} · Underflows {snapshot.underflowCallbacks}</small>
  </article>;
}
...
function App() {
  ...
  return <main className="app-shell">
    ...
    <section className="deck-grid">
      <DeckPanel id="a" snapshot={mixer.deckA} seek={seekA} gain={mixer.channelGainA} cued={mixer.cueA} busy={busyDeck === "a"} message={messageA} onSeekChange={setSeekA} onCommand={(command, args, success) => runDeckCommand("a", command, args, success)} onGain={(gain) => { setMixer({ ...mixer, channelGainA: gain }); void setMixerControl("deck_a_set_gain", { gain }); }} />
      ...
      <DeckPanel id="b" snapshot={mixer.deckB} seek={seekB} gain={mixer.channelGainB} cued={mixer.cueB} busy={busyDeck === "b"} message={messageB} onSeekChange={setSeekB} onCommand={(command, args, success) => runDeckCommand("b", command, args, success)} onGain={(gain) => { setMixer({ ...mixer, channelGainB: gain }); void setMixerControl("deck_b_set_gain", { gain }); }} />
    </section>
